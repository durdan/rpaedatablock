...
public class TransactionResource {
    ...
    @Timed
    public ResponseEntity<List<TransactionDTO>> getAllTransactions(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("ocrprocessingerror-is-null".equals(filter)) {
            log.debug("REST request to get all Transactions where ocrProcessingError is null");
            return new ResponseEntity<>(transactionService.findAllWhereOcrProcessingErrorIsNull(),
                    HttpStatus.OK);
        }
